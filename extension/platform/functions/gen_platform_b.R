gen_platform_b <-
  function(treat_response_A,
    ...
    # Defining n_A's, total number of patients per section in time:
    n_A1 <- n_arm
    n_A2 <- (n_arm)*2
    n_A3 <- n_arm
    
    # Sum of outcomes for treatment and control groups:
    treatment_A_a1 <- rbinom(n = 1, size = n_A1/2, prob = treat_response_A) # Treatment arm 1 (A), section A1
    treatment_A_a2 <- rbinom(n = 1, size = n_A2/4, prob = treat_response_A) # Treatment arm 1 (A), section A2
    treatment_B_a2 <- rbinom(n = 1, size = n_A2/4, prob = treat_response_B) # Treatment arm 2 (B), section A2
    treatment_B_a3 <- rbinom(n = 1, size = n_A3/2, prob = treat_response_B) # Treatment arm 2 (B), section A3
    
    control_group_A1 <- rbinom(n = 1, size = n_A1/2, prob = baseline_response)  # Control group per section in time (ratio 1:1)
    control_group_A2 <- rbinom(n = 1, size = n_A2/2, prob = baseline_response) # Control group per section in time (ratio 2:1:1)
    control_group_A3 <- rbinom(n = 1, size = n_A3/2, prob = baseline_response) # Control group per section in time (ratio 1:1)
    ...
    n_test_1 <- c(n_A1/2 + n_A2/4, n_A1/2 + n_A2/2)
    n_test_2 <- c(n_A2/4 + n_A3/2, n_A2/2 + n_A3/2)
