...
colnames(df2) <- c("Treatment","", "A","A", rep(c("A"," "), 6), rep(c("A", "B", ""), 5))
...
table_html <- kable(df2, format = "html", caption = "Table of results 2: Powered to delta = 10%, with baseline response rate 30% (N_arm = 498).", align = "c", escape = FALSE) %>%
...
table_latex <- kable(df2, format = "latex", caption = "Table of results 2: Powered to delta = 10%, with baseline response rate 30% (N_arm = 498).", align = "c", escape = FALSE) %>%
  add_header_above(third_header) %>%
  add_header_above(second_header) %>%
  add_header_above(top_header) %>% 
  kable_styling(full_width = FALSE)
