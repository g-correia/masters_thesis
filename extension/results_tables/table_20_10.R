...

{
  ...
  for(j in 1:settings_a){
    df[2,4 + j*2 -2] <- unname(rejection_1_3[[j]][1]) # A
    df[2,4 + j*2 -1] <- unname(discovery_1_3[[j]][1]) # FDR
  }
  ...
  ##############
  for(j in 1:settings_a){
    df[3,4 + j*2 -2] <- unname(rejection_2_3[[j]][1]) # A
    df[3,4 + j*2 -1] <- unname(discovery_2_3[[j]][1]) # FDR
  }
  ...
  ##############
  for(j in 1:settings_a){
    df[4,4 + j*2 -2] <- unname(rejection_3_3[[j]][1]) # A
    df[4,4 + j*2 -1] <- unname(discovery_3_3[[j]][1]) # FDR
  }
  ...
  ##############
  for(j in 1:settings_a){
    df[5,4 + j*2 -2] <- unname(rejection_4_3[[j]][1]) # A
    df[5,4 + j*2 -1] <- unname(discovery_4_3[[j]][1]) # FDR
  }
  ...
  ##############
  for(j in 1:settings_a){
    df[6,4 + j*2 -2] <- unname(rejection_5_3[[j]][1]) # A
    df[6,4 + j*2 -1] <- unname(discovery_5_3[[j]][1]) # FDR
  }
  ...
  ##############
  for(j in 1:settings_a){
    df[7,4 + j*2 -2] <- unname(rejection_6_3[[j]][1]) # A
    df[7,4 + j*2 -1] <- unname(discovery_6_3[[j]][1]) # FDR
  }
  ...
  ##############
  for(j in 1:settings_a){
    df[8,4 + j*2 -2] <- unname(rejection_7_3[[j]][1]) # A
    df[8,4 + j*2 -1] <- unname(discovery_7_3[[j]][1]) # FDR
  }
  ...
  ##############
  for(j in 1:settings_a){
    df[9,4 + j*2 -2] <- unname(rejection_8_3[[j]][1]) # A
    df[9,4 + j*2 -1] <- unname(discovery_8_3[[j]][1]) # FDR
  }
  ...
  ##############
  for(j in 1:settings_a){
    df[10,4 + j*2 -2] <- unname(rejection_9_3[[j]][1]) # A
    df[10,4 + j*2 -1] <- unname(discovery_9_3[[j]][1]) # FDR
  }
...
table_html <- kable(df, format = "html", caption = "Table of results 3: Powered to delta = 20%, with baseline response rate 10% (N_arm = 93).", align = "c", escape = FALSE) %>%
...
table_latex <- kable(df, format = "latex", caption = "Table of results 3: Powered to delta = 20%, with baseline response rate 10% (N_arm = 93).", align = "c", escape = FALSE) %>%
  add_header_above(third_header) %>%
  add_header_above(second_header) %>%
  add_header_above(top_header) %>% 
  kable_styling(full_width = FALSE)
